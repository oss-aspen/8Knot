...
This guide covers the **dramatically faster** development build workflow for 8Knot, featuring optimized Docker/Podman builds and lightning-fast startup times.
...
When you run `./quick-build.sh`, it combines both compose files where the dev file overrides the production settings for development speed.
...
**Note:** The main `docker-compose.yml` remains unchanged for CI/CD compatibility. Development optimizations are in `docker-compose.dev.yml`.
