...
def create_graph(data,interval):
    ...
    if(df_issues is not None):
        fig = go.Figure()
        fig.add_histogram( x=df_issues["closed"].dropna(), histfunc = 'count', name= "closed",opacity=1
                            , hovertemplate =hover+ "<br>Closed: %{y}<br>"+ '<extra></extra>')
        fig.add_histogram(x=df_issues["created"] ,histfunc = 'count', name= "created",opacity=.6
                            , hovertemplate =hover+ "<br>Created: %{y}<br>"+ '<extra></extra>')
        fig.update_traces(xbins_size=interval)
        fig.update_xaxes(showgrid=True, ticklabelmode="period", dtick=interval,rangeslider_yaxis_rangemode="match" , range=x_r)
        fig.update_layout(
            title={'text':"Issues Over Time",
                      'font':{'size':28},'x':0.5,'xanchor':'center'},
        xaxis_title=x_name,
        yaxis_title="Number of Issues",
        barmode="overlay",
        )
        fig.add_trace(go.Scatter(x=df_open["issue"], y=df_open["total"],
                    mode='lines',
