...
def create_contrib_prolificacy_over_time_graph(repolist, patterns, threshold, window_width, step_size, start_date, end_date):
...
def process_data(df, patterns, threshold, window_width, step_size, start_date, end_date):
    ...
    df_final['PR Opened'],
    df_final['PR Comment'], 
    df_final['PR Review']) = zip(*df_final.apply(lambda row: cntrb_prolificacy_over_time(df, row.period_from, row.period_to, window_width, threshold), axis=1))
    
    return df_final

def create_figure(df_final, step_size):
    # create custom data to update the hovertemplate with the start and end dates of a given time window
    ...
    fig = go.Figure(
        [
            ...
            go.Scatter(name = 'PR Request Comment',
                       x = df_final['period_from'],
            ...
            go.Scatter(name = 'PR Request Review',
                       x = df_final['period_from'],
    ...
    fig.update_traces(textposition="top right", 
                      hovertemplate= "Date: %{customdata}" +"<br>Contributor Prolificacy: %{y}<br><extra></extra>",
                      )
...
def cntrb_prolificacy_over_time(df, period_from, period_to, window_width, threshold):
    ...
    commit, issueOpened, issueClosed, prOpened, prReview, prComment = None, None, None, None, None, None
    ...
    issueClosed = calc_cntrb_prolificacy(df_count_cntrbs, 'Issue Opened', threshold)
    ...
    return commit, issueOpened, issueClosed, prOpened, prReview, prComment
