...
def graph_title(view):
    title = ""
    if view == -1:
        title = "Total Stars Over Time"
    elif view == "M":
        title = "New Stars by Month"
    else:
        title = "New Stars by Year"
    return title
...
def star_count_graph(repolist, interval):
    ...
    logging.warning("STAR_COUNT_VIZ - START")
    start = time.perf_counter()

    # test if there is data
    if df.empty:
        logging.warning("STAR_COUNT_VIZ - NO DATA AVAILABLE")    
        return nodata_graph
    ...
    logging.warning(f"STAR_COUNT_VIZ - END - {time.perf_counter() - start}")
    return fig
...
def process_data(df, interval):
    ...
    logging.warning(df2.head())

    if interval == -1:
        return df2, None

    # get the count of new contributors in the desired interval in pandas period format, sort index to order entries
    created_range = pd.to_datetime(df2["created"]).dt.to_period(interval).value_counts().sort_index()

    # converts to data frame object and creates date column from period values
    df2_contribs = created_range.to_frame().reset_index().rename(columns={"index": "Date", "created": "contribs"})

    # converts date column to a datetime object, converts to string first to handle period information
    df2_contribs["Date"] = pd.to_datetime(df2_contribs["Date"].astype(str))

    # correction for year binning -
    # rounded up to next year so this is a simple patch
    if interval == "Y":
        df2_contribs["Date"] = df2_contribs["Date"].dt.year
    elif interval == "M":
...
def create_figure(df, df_contribs, interval):
    ...
    if interval == -1:
        ...
        fig.update_traces(hovertemplate="Stars: %{y}<br>%{x|%b %d, %Y} <extra></extra>")
    else:
        fig = px.bar(
            df_contribs,
            x="Date",
            y="contribs",
            range_x=x_r,
            labels={"x": x_name, "y": "Stars"},
            color_discrete_sequence=[color_seq[3]],
        )
        fig.update_traces(hovertemplate=hover + "<br>Stars: %{y}<br>")
