...
def fuzzy_search(query: str, options: List[Dict[str, Any]], threshold: float = 0.2) -> List[Dict[str, Any]]:
    ...
    # For very short queries (1-2 chars), a simpler and faster matching approach
    if len(query) <= 2:
        ...
        for option in options:
            # Using pre-computed lowercase label if available, otherwise compute it
            if '_label_lower' in option:
                label_lower = option['_label_lower']
            else:
                label_lower = option["label"].lower()
                # Cache it for later use
                option['_label_lower'] = label_lower
    ...
    filtered_options = []
    for option in options:
        # Get or compute lowercase label
        if '_label_lower' in option:
            label_lower = option['_label_lower']
        else:
            label_lower = option["label"].lower()
