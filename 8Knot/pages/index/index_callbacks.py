...
import dash
from dash import callback, html, ctx, clientside_callback
...
#     app stores list.
#   * The load path NEVER writes `repo-choices` directly. It sets
#     `projects.value` and bumps `search-button.n_clicks`, reusing the
#     existing `multiselect_values_to_repo_ids` pipeline. This avoids
#     `allow_duplicate` races on `repo-choices`.
...
def _extract_base_url(href: str) -> str:
...
def manage_share_modal(share_clicks, close_clicks, repo_ids, pathname, href):
    ...
    base_url = _extract_base_url(href)
    ...
    try:
        ...
        share_url = url_utils.compose_short_url(base_url, short_id, pathname, graph_id)
        return True, share_url, "", ""
    except Exception as e:
        # Layer-1 fallback: a self-contained long URL still works without the DB.
        ...
        return True, long_url, "Short-link service unavailable — using a full link.", ""
...
def handle_share_url(search):
    ...
    # Resolve repo_ids against THIS instance. `repo_id_to_git` is an in-memory
    # dict lookup (cheap) that returns None for repos this instance doesn't
    # know about — which is exactly how we detect "missing" repos and honor
    # the "load remaining + warn the user" behavior.
    repo_ids = state.get("repo_ids", [])
    options, missing = [], []
    for rid in repo_ids:
        git_url = augur.repo_id_to_git(rid)
        if git_url:
            # dmc.MultiSelect uses string values and only keeps values that
            # also appear in `data`, so we MUST emit a matching option for
            # each value or the selection is silently discarded.
