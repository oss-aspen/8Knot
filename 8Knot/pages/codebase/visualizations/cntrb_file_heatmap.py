        ...
        try:
            ...
            if not df.empty:
                ...
                if repo_name and repo_path and repo_name != "None" and repo_path != "None":
                    ...
                    logging.warning(f"cntrb-file-heatmap - Selected repo {repo_id} with valid metadata")
                    break
                else:
                    logging.warning(
                        f"cntrb-file-heatmap - Skipping repo {repo_id} with NULL metadata: repo_name='{repo_name}', repo_path='{repo_path}'"
                    )
        except Exception as e:
            logging.warning(f"cntrb-file-heatmap - Error checking repo {repo_id}: {e}")
            continue

    # Fall back to first repo if none have valid metadata
    if default_repo is None:
        default_repo = repo_ids[0] if repo_ids else None
        logging.warning(f"cntrb-file-heatmap - No valid repos found, falling back to {default_repo}")
...
def df_file_clean(df_file: pd.DataFrame, df_file_cntbs: pd.DataFrame, bot_switch):
    ...
    if not repo_name or not repo_path or repo_id_raw is None or repo_name == "None" or repo_path == "None":
        logging.warning(
            f"{VIZ_ID} - Null or empty values in repo metadata: repo_name='{repo_name}', repo_path='{repo_path}', repo_id={repo_id_raw}"
        )
        return pd.DataFrame()
