...
def reviewer_file_heatmap_graph(searchbar_repos, repo_id, directory, bot_switch):
    ...
    # process data using shared function (reviewer IDs)
    df = process_activity_heatmap(df_file, df_actions, df_file_cntbs, directory, bot_switch, "reviewer_ids")

    # if there are no reviewers in a directory plot no data graph
    if df.empty:
        return nodata_graph
